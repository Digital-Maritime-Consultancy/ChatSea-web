...
export const parseS124 = async (xmlString: string): Promise<S100Data> => {
  ...
  if (!members || !members.NAVWARNPart || !members.NAVWARNPreamble) throw new Error('S124 Data is not valid');
  ...
  if (parsed_polygons.length === 0 && parsed_markers.length === 0) {
    throw new Error('No valid NAVWARNPart elements found.');
  }
